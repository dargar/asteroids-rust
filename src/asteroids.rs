...
impl Asteroids {
    pub fn new() -> Asteroids {
        Asteroids {
            ...
            vao: 0,
            projection: cgmath::ortho(0.0f32, 800.0, 600.0, 0.0, -1.0, 1.0),
...
pub fn update_and_render(asteroids: &mut Asteroids, input: &[char]) {
    ...
    for &event in input {
        match event {
            ...
            'a' => asteroids.entity.direction += -2.0,
            'd' => asteroids.entity.direction += 2.0,
            _ => (),
        }
    }

    asteroids.entity.update(1.0 / 60.0, a);
